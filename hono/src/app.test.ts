...
await test('serveStatic', async (t) => {
	...
	await t.test('CSP', async () => {
		...
		assert.equal(
			...
			"base-uri 'none';form-action 'self' https://www.google.com https://www.bing.com https://search.yahoo.co.jp https://duckduckgo.com;frame-ancestors 'self';report-uri https://api.w0s.jp/report/csp;report-to default",
		);
		assert.equal(
