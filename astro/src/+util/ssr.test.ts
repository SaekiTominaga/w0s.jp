...
await test('getPageUrl', async (t) => {
	await t.test('top page', () => {
		assert.equal(getPageUrl(new URL('http://example.com/'), '/var/www/index.astro'), '/');
	});
	...
	await t.test('normal page', async (t2) => {
		...
		await t2.test('build', () => {
			assert.equal(getPageUrl(new URL('http://example.com/path/to.html'), '/var/www/path/to.astro'), '/path/to');
		});
	});
});
